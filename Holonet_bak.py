...
Nxy = 64
iter_num = 9
data_dir = "./sim_train_data_5_rand/"
out_dir = "./output_5_rand/"
...
class HOLONET():
    ...
    # def residual_block(self, layer_input, filters):  # denoising network
        ...
        # d_sym_diff = tf.keras.layers.Lambda(lambda x: tf.math.add(x[0], x[1]))([d_real_sym_diff, d_imag_sym_diff])
    ...
    def residual_block(self, layer_input, filters):  # denoising network
        ...
        Conv1 = Conv2D(filters, kernel_size=3, strides=1, activation='relu', use_bias=True,
                       padding='same', data_format='channels_first', kernel_initializer='zeros')
        Conv2 = Conv2D(filters, kernel_size=3, strides=1, activation='relu', use_bias=True,
                       padding='same', data_format='channels_first', kernel_initializer='zeros')

        d_real2 = Conv2(Conv1(d_real))
        d_imag2 = Conv2(Conv1(d_imag))
		

        d_abs = tf.keras.layers.Lambda(lambda x: tf.math.abs(tf.complex(x[0], x[1])))(
            [d_real2, d_imag2])
        minus_d = MyLayer((Nz, Nxy, Nxy))(d_abs)

        minus_d = Activation('relu')(minus_d)

        d_scale = tf.keras.layers.Lambda(lambda x: tf.sign(x))(minus_d)

        d_real = tf.keras.layers.Lambda(lambda x: tf.multiply(x[0], x[1]))([d_scale, d_real2])
        d_imag = tf.keras.layers.Lambda(lambda x: tf.multiply(x[0], x[1]))([d_scale, d_imag2])

        Conv3 = Conv2D(filters, kernel_size=3, strides=1, activation='relu', use_bias=True,
                       padding='same', data_format='channels_first', kernel_initializer='zeros')
        Conv4 = Conv2D(filters, kernel_size=3, strides=1, use_bias=True, padding='same',
                       data_format='channels_first', kernel_initializer='zeros')
        ...
        d_real_sym = Conv4(Conv3(d_real2))
        d_imag_sym = Conv4(Conv3(d_imag2))
...
#       d_relative_diff = tf.keras.layers.Lambda(lambda x:  x[0]/(1e-8 + tf.reduce_sum(tf.math.real(x[1]) + tf.math.imag(x[1])))+ x[0]/(1e-8+tf.reduce_sum(x[2] + x[3])))([d_sym_diff,d,d_real_sym,d_imag_sym])
    ...
    def get_holonet(self, train_bn=True):
        ...
        def depthscalelayer(inputimg):
            # complex
            img_real = tf.keras.layers.Lambda(lambda x: tf.real(x))(inputimg)
            img_imag = tf.keras.layers.Lambda(lambda x: tf.imag(x))(inputimg)
            scale = tf.keras.layers.DepthwiseConv2D(1, use_bias=False, data_format='channels_first')#, kernel_constraint= tf.keras.constraints.NonNeg())
        ...
        def scalelayer(inputimg):
            ...
            scale = tf.keras.layers.Conv3D(1,1,use_bias= False, kernel_initializer='zeros', kernel_constraint= tf.keras.constraints.NonNeg())
            ...
            temp_real = tf.keras.layers.Lambda(lambda x: tf.squeeze(x,-1))(temp_real)
            temp_imag = tf.keras.layers.Lambda(lambda x: tf.squeeze(x,-1))(temp_imag)
...
#        response = tf.keras.layers.GaussianNoise(0.01)(response)
        # get initial guess
        ...
        tempv_real = Activation('sigmoid')(tempv_real)
        tempv_imag = Activation('sigmoid')(tempv_imag)
        ...
        def Myloss(symloss):
            def m_loss(y_true, y_pred):
                ...
                sumtrue = tf.math.abs(tf.real(y_true) + tf.imag(y_true)) +1e-10
                sumpred = tf.math.abs(tf.real(y_pred) + tf.imag(y_pred)) +1e-10
                tempdiff = (tf.math.reduce_mean(tempreal) + tf.math.reduce_mean(tempimag)) \
                           + tf.math.reduce_mean(symloss) / 10 / iter_num
                return tempdiff
    ...
    def load_data(self, testtime):
        if testtime == True:
            ...
            for i in range(num_sample + 0, num_sample + num_test):
                mat = h5py.File(data_dir + str(i + 1) + ".mat")
    ...
    def load_kernel(self):
        mat = hdf5storage.loadmat(data_dir + "otf3d.mat")
        otf3d = mat['otf3d']
...
#        model.load_weights(out_dir + 'holonet.hdf5')
        model_checkpoint = ModelCheckpoint(out_dir + 'holonet.hdf5',
										   monitor='loss',
										   verbose=1,
                                           save_best_only=True)
        ...
        tempreal = np.mean(np.abs(np.real(imgs_train) - np.real(img_predict)))
        tempimag = np.mean(np.abs(np.imag(imgs_train) - np.imag(img_predict)))
		
        print(tempreal+tempimag)
        scipy.io.savemat(out_dir + 'predict.mat', {'predict': img_predict})
        scipy.io.savemat(out_dir + 'gt.mat', {'gt': imgs_train})
